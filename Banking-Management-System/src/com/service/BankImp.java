        ...
        try {
            ...
            if (rowsAffected > 0) {
                ...
                if (rowsAffected > 0) {
                    ...
                    System.out.print("Saving Account intrest Rate: ");
...
// ----------------------------------------------------------------------- Customer Start------------------------------------------------------------------
	    ...
	    try {
	        ...
	        if (resultSet.next()) {
	            ...
	            if (rowsAffected > 0) {
	                ...
	                String insertTransactionQuery = "INSERT INTO transaction (datetime, amount, customer_id, transaction_type) VALUES (NOW(), ?, ?, ?)";
	...
	public boolean transferMoney() throws SQLException {
	    ...
	    // Set the transaction type to "transfer"
	    String transactionType = "transfer from "+ sourceAccountId +" to "+ destinationAccountId;
	...
	public void viewTransactionHistory() throws SQLException {
	    ...
	    try {
	        ...
	        String selectQuery = "SELECT id, datetime, amount, transaction_type FROM transaction WHERE customer_id = ?";
	        PreparedStatement preparedStatement = con.prepareStatement(selectQuery);
	        preparedStatement.setInt(1, customerId);
	        ...
	        while (resultSet.next()) {
	            ...
	            System.out.println("Transection Type: " + transactionType);
	            System.out.println("-----------------------------");
	        }
	    } catch (SQLException e) {
	        e.printStackTrace();
